...
Meal.addFoodstoMealTable = function(event) {
	...
	selectedFoods.forEach(function(food) {
		if (food.children[2].children[0].checked) {
			selectedFoodIDS.push(food.children[0].dataset.id)
		}
	})
	Meal.updateMeals(mealID, selectedFoodIDS)
	var mealTable = event.currentTarget.innerText.toLowerCase()
	Meal.prependFood(mealTable, selectedFoodIDS)
}
...
Meal.updateMeals = function(meal, foods) {
	...
	foods.forEach(function(food) {
		$.ajax({
			type: "POST",
			url: `https://shrouded-headland-61661.herokuapp.com/api/v1/meals/${mealID}/foods/${food}`,
			dataType: "json",
		})
		// Meal.clearTables()
		// Meal.allMealsToHTML()
...
Meal.prependFood = function(mealTable, FoodIDs) {
	...
	FoodIDs.forEach(function(food) {
		var rawFood = Food.getAFood(food)
		var foodObject = new Food(rawFood)
...
// Meal.clearTables = function() {
// 	$('.diary-table-breakfast .food-row').remove();
// 	$('.diary-table-snacks .food-row').remove();
// 	$('.diary-table-lunch .food-row').remove();
// 	$('.diary-table-dinner .food-row').remove();
// }
