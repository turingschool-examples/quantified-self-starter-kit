...
function addFood(id, name, calories) {
        $('table').append(`<tr class="food-${id}"><td>${name}</td><td>${calories}</td><td><img class="delete-food" data-id="${id}" src="./lib/assets/delete1.png"></td></tr>`);
...
function sendPostRequest(name, calories) {
  ...
  .then(data => addFood(data.id, data.name, data.calories))
}
