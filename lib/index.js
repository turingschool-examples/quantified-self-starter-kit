...
$('#new_foods').submit(function(event) {
  ...
  if (!$('#food_name').val()) {
    $('#errorMsg').html('Error: Please enter a name')
    // $('food_name').append('<span class="error">Please enter a food name. </span>')
  }

  if (!$('#food_calories').val()) {
    debugger
    $('#caloriesErrorMsg').html('Error: Please enter calories')
    // $('food_calories').append('<span class="error">Please enter a calorie amount. </span>')
  }
