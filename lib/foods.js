let localURL = "http://localhost:1234"
        ...
        .then(function(foods){
          ...
          foods.forEach(function(food){
              $("#foodTable").find('tbody')
                .append($(`<tr class=list id=${food.id} >`)
...
function deleteFoodMeal(itemId){
        ...
        $.get(`${localURL}/api/v1/meals`)
          .then(function(meals){
            debugger;
            meals.forEach(function(meal) { //can't pass itemId to loop; it's looking for index; resets foodId to 0,1...
              if (meal.foods.ids == itemId) {
                    let mealId = meal.id;
                    debugger;
                    $.ajax({ // start delete join record
