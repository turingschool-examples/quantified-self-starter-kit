// $(document).ready(function(){
  ...
  // $.post("http://localhost:3000/api/v1/foods?name=new-food&calories=new-food-calories");
  ...
  data: { food: { name: "Orange", calories: "300"} }
  ...
  $.then(function(response){
    $("#foodTable").find('tbody')
      .append($('<tr class=list>')
        .append($(`<td class=edit-name contenteditable=true>${response.name}</td> <td class=edit-calories contenteditable=true>${response.calories}</td><td><input type=button name=delete-food id=${data.id} value=delete </td>`)
  ))})
  //     dataType: JSONP
  //   });
  // }
// });{"food"=>"", "name"=>"new-food", "calories"=>"new-food-calories"}
