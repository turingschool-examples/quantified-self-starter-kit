...
function addToBreakfastTable(){
  $(".breakfast").on("click", function(){
    collectTableSelection()
    removeCheckedBoxes()
    updateTableContents()
  })
}
...
function collectTableSelection(){
  $.each($("#foodTable :checkbox:checked"), function(){
    $.post("http://localhost:3000/api/v1/meals/1/foods/" + ($(this).closest("tr").find("td:eq(3)").children().attr("id")), function(data){
    })
  })
}
